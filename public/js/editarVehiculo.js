...
var miVariableColor = document.getElementById('miVariableColor').getAttribute('data-color');
var tipos = JSON.parse(miVariable);
var colores = JSON.parse(miVariableColor);

console.log('hola Vehiculo');
...
tabla.addEventListener('click', function (event) {
    ...
    if (event.target.classList.contains('activar')) {
        ...
        var ventanaEdit = document.getElementById('editar');
        var cerrar = document.getElementById('edit');

        var formularioEdit = document.getElementById('formeditar');
        var nuevaURL = formularioEdit.getAttribute('data-action');

        var nuevaCadena = nuevaURL.slice(0, nuevaURL.lastIndexOf('/'));
        console.log(nuevaCadena + '/' + idcamion);
        formularioEdit.action = nuevaCadena + '/' + filaInfo.id;
...
console.log(formularioEdit);
    ...
    <div class="divContenedoredit overflow-auto custom-scroll" style="max-height: 500px; overflow-x: auto;">
        <div class="divInfoedit mt-3">
            ...
            <div class="row mb-2">
                <label for="color" class="col-md-3 col-form-label text-md-start">Color Vehiculo</label>
                <div class="col-md-8">
        ...
        tipos.forEach(function (tipo) {
          ...
          var seleccolores = document.getElementById('color');
        colores.forEach(function (color) {
            var option = document.createElement('option');
            option.value = color.id;
            option.textContent = color.name;
            seleccolores.appendChild(option);
        });

        ventanaEdit.style.display = 'block'; // Mostrar la ventana oculta
